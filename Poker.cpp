...
int main()
{
    ...
    while (true) {
        ...
        if (cash[indexSmallBlinde] >= blinde) {
            cash[currentPlayer++] -= blinde;
        }
        for (int i = 0; i < indexSmallBlinde, i < playersCount; i++) {
            cout << "enter your blinde: ";
            int currentBlinde;
            cin >> currentBlinde;
            if (currentBlinde == 0) {
